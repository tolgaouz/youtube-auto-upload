...
def chrome_cookies(
    ...
    # here is some change, return a list instead of a dictionary
    cookies = []
    curl_cookies = []
    ...
    for host_key in generate_host_keys(domain):
        for (
            ...
            # create a temporary dictionary for each cookie
            tmp = {}
            # edit in the way that we can pass it directly to selenium webdriver
            tmp['name'] = cookie_key
            tmp['value'] = val
            tmp['domain'] = host_key
            tmp['secure'] = bool(is_secure)
            tmp['expiry'] = expires_utc
            tmp['path'] = path
